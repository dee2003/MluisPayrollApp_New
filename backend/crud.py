...
# 🔹 Update
def update_material_trucking(db: Session, id: int, update_data: schemas.MaterialTruckingUpdate):
    ...
    # Update materials link
    if update_data.material_ids is not None:
        materials = db.query(models.Material).filter(models.Material.id.in_(update_data.material_ids)).all()
...
# 🔹 Update Dumping Site
def update_dumping_site(db: Session, site_id: str, site_data: schemas.DumpingSiteCreate):
    ...
    site.dumping_type = site_data.dumping_type
    site.dumping_category = site_data.dumping_category
    site.status = site_data.status.upper() if site_data.status else site.status
