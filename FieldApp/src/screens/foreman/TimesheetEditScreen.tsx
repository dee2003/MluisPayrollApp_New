...
// Inside your TimesheetEditScreen component, after all the useState hooks:
  ...
  useEffect(() => {
    ...
    materialTickets, vendorTickets, dumpingSiteTickets, notes, totalQuantities, 
  ...
  const fetchData = async () => {
    try {
      ...
      if (savedDataJSON !== null) {
          ...
          setTotalQuantities(savedData.totalQuantities || {});
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedData = {
        ...
        dumping_sites: timesheet.data.dumping_sites?.map(site => ({
          ...site,
                  ...
                  {isEquipment ? (
                    ...
                    <>
                      <View style={styles.inputWithLabel}>
                        ...
                        <TextInput
                          style={[styles.input, (isTicketsDisabled || (isDumpingSite && selectedPhase !== firstPhase)) && styles.disabledInput]}
                          keyboardType="number-pad"
                          placeholder="0"
                          value={(ticketsState as SimpleHourState)[entity.id]?.[selectedPhase ?? ''] ?? ''}
                          editable={!isTicketsDisabled && !(isDumpingSite && selectedPhase !== firstPhase)}
                          onChangeText={text => selectedPhase && handleSimpleValueChange(type as 'material' | 'vendor' | 'dumping_site', 'tickets', entity.id, selectedPhase, text)}
                        />
                      </View>
                      <View style={styles.inputWithLabel}>
                        <Text style={styles.inputHeader}>{getHeader('hours')}</Text>
                        <TextInput
                          style={[styles.input, isLoadsDisabled && styles.disabledInput]}
                          keyboardType="numeric"
                          placeholder="0"
                          value={(hoursState as SimpleHourState)[entity.id]?.[selectedPhase ?? ''] ?? ''}
                          editable={!isLoadsDisabled}
                          onChangeText={text => selectedPhase && handleSimpleValueChange(type as 'material' | 'vendor' | 'dumping_site', 'hours', entity.id, selectedPhase, text)}
                        />
                      </View>
                      {(isMaterial || isVendor) && (
                        <View style={styles.inputWithLabel}>
