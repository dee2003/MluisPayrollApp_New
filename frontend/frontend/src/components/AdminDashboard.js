...
//         switch (type) {
//             case "user": return [  { name: "id", label: "User ID", required: true },{ name: "username", label: "Username", required: true }, { name: "first_name", label: "First Name", required: true }, { name: "middle_name", label: "Middle Name" }, { name: "last_name", label: "Last Name", required: true }, { name: "email", label: "Email", required: true, type: "email" }, { name: "password", label: "Password", type: "password", required: true }, { name: "role", label: "Role", type: "select", options: [ { value: "foreman", label: "Foreman" }, { value: "supervisor", label: "Supervisor" }, { value: "project_engineer", label: "Project Engineer" }, { value: "admin", label: "Accountant" } ], required: true, defaultValue: "admin" },  { name: "status", label: "Status", type: "select", options: [ { value: "Active", label: "Active" }, { value: "Inactive", label: "Inactive" } ], required: true, defaultValue: "Active" }];
...
//     // If extra is a string, treat it as label (old behavior)
//     // If extra is an object, treat it as extra props (new behavior)
...
//   return makeTableWithPagination(
//     "job_phase",
//     "Jobs & Phases Management",
//     ["Job Code", "Description", "Project Engineer", "Status"],
//     job => (
//       <>
//         <td>{job.job_code}</td>
...
// --- Generic Form Component (Unchanged) ---
const GenericForm = ({ fields,vendorOptions, fetchVendorOptions, onSubmit, defaultValues = {}, errorMessage,categories = [] }) => {
    const [formData, setFormData] = useState(defaultValues);
    const [values, setValues] = useState(() => {
        const initialValues = { ...defaultValues };
        fields.forEach(field => {
...
//     const BASE_URL = "http://localhost:8000"; // 👈 backend port

//     const types = await axios.get(`${BASE_URL}/api/vendor-options/type`);
//     const categories = await axios.get(`${BASE_URL}/api/vendor-options/category`);
//     const materials = await axios.get(`${BASE_URL}/api/vendor-options/material`);
//     const units = await axios.get(`${BASE_URL}/api/vendor-options/unit`);
...
const handleToggleStatus = async (type, item, newStatus) => {
 ...
 case "equipment":
  ...
  //         label="Vendor Material"
  //         type="material"
  //         options={vendorOptions.material.map(v => ({ value: v, label: v }))}
  //         value={form.vendor_material || ""}
  //         onChange={(e) => setForm({ ...form, vendor_material: e.target.value })}
  //         reloadOptions={fetchVendorOptions}
  //       />
  //     ),
  //   },
  ...
  //         label="Unit"
  //         type="unit"
  //         options={vendorOptions.unit.map(v => ({ value: v, label: v }))}
  //         value={form.unit || ""}
...
//   label: "Materials & Units",
//   type: "materials_block", // 👈 not 'custom'
// },
{
  name: "materials",
